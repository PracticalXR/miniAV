...
/*
 * Establishes cross-origin isolation (COOP: same-origin, COEP: require-corp) so
 * SharedArrayBuffer is available — required by the miniav AudioWorklet WASM
 * build (and by minigpu's WebGPU shared-memory build). On hosts where you can
 * set real response headers (e.g. Firebase Hosting via firebase.json), those
 * headers are preferred and this shim is a harmless no-op fallback.
 */
